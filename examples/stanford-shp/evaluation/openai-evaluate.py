...
parser.add_argument("--model", default="gpt-3.5-turbo", choices=["gpt-4", "gpt-3.5-turbo", "gpt-4-1106-preview"])
...
MODEL_COSTS = {"gpt-4": [0.00003, 0.00006], "gpt-3.5-turbo": [0.0000015, 0.000002], "gpt-4-1106-preview": [0.00001, 0.00003]}
...
with open(args.output_file1) as fout1, open(args.output_file2) as fout2, open(args.results_file, "w") as fres:
    for idx, line1 in tqdm(enumerate(fout1)):
        ...
        # print(responsedata)
        # input()
        # print("YES",    requestdata['response'])
        # input()
        fres.write(json.dumps(responsedata)+"\n")
        fres.flush()

        if "gpt-4" in args.model and count > 2000:
            break
