...
class ScriptArguments:
    ...
    save_model_dir: Optional[str] = field(default="/projects/tir6/general/sachink/personalized-LM/2023/models/rlhf-new/", metadata={"help": "the reward model name"})
    ...
    reward_tokenizer_name: Optional[str] = field(default="microsoft/deberta-v3-large", metadata={"help": "the reward model tokenizer name"})
...
def build_shp_dataset(tokenizer, data_dir):
    logging.warning("starting data load")
    # split_to_load="validation"
    split_to_load="train"
    if split_to_load == "validation":
        logging.warning("BEWARE! You are using validation data for training....")
    ds = load_dataset("stanfordnlp/shp", split=split_to_load)

    if data_dir != "plain" and data_dir != "contextualized" and data_dir != "subredditname":
    ...
    def tokenize(sample):
        ...
        modelinstruction_tokenized = tokenizer.encode(modelinstruction)
        max_length = 500
        remaining = max_length - len(modelinstruction_tokenized)
        sample_tokenized = tokenizer.encode(clean_text(sample["history"]))[:remaining] 
        sample['query'] = domaindescription + tokenizer.decode(sample_tokenized)
        x = modelinstruction_tokenized + sample_tokenized + tokenizer.encode(" \n\n RESPONSE: ")
...
reward_tokenizer_name = script_args.reward_tokenizer_name if script_args.reward_tokenizer_name is not None else script_args.reward_model_name
print(reward_tokenizer_name)

reward_model_tokenizer = AutoTokenizer.from_pretrained(reward_tokenizer_name)
reward_model_tokenizer.pad_token = tokenizer.eos_token
reward_model = transformers.AutoModelForSequenceClassification.from_pretrained(reward_model_name).to(device).half()
...
def reward_function(query, response, **sent_kwargs):
    with torch.no_grad():
        ...
        texts = [f"{q} \n\n RESPONSE: {r}" for q, r in zip(query, response)]
        # for text in texts:
            # print(text)
            # input()
        input_ids = reward_model_tokenizer(texts, padding=True, return_tensors="pt").input_ids.to(device)  
        # print(input_ids[:, :25])
        # print(input_ids.size())
        outputs = reward_model(input_ids[:, :25])

        scores = outputs.logits
        # print(scores.size())
        # torch.set_printoptions(precision=15)
        # print(scores)
        # print(scores * 1000000)
        # torch.set_printoptions(precision=5)

        new_scores = []
        for i in range(scores.size(0)):
            new_scores.append(scores[i])#[scores[i] for ]    
...
if reward_model_name.endswith("/"):
    reward_model_name = reward_model_name[:-1]
