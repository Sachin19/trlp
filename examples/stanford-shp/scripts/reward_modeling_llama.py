...
class ScriptArguments:
    ...
    data_dir: Optional[str] = field(default="stanfordnlp/shp", metadata={"help": "the model name, write all for all domains combined"})
...
if script_args.data_source == "SO":
    ...
    if script_args.data_source == "chp":
        load_dataset = load_from_disk

    print(script_args.subset)
    if script_args.subset == "all":
        train_dataset = load_dataset(load_from_disk, split="train")
    else:
        train_dataset = load_dataset(load_from_disk, split="train", data_dir=script_args.subset)
    ...
    if script_args.subset == "all":
        eval_dataset = load_dataset(load_from_disk, split="validation")
    else:
        eval_dataset = load_dataset(load_from_disk, split="validation", data_dir=script_args.subset)
