...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        eval_dataset: Optional[Union[torch.utils.data.Dataset, Dataset]] = None,
        ...
        if self.eval_dataset is not None:
            self.eval_dataloader = self.prepare_dataloader(self.eval_dataset, data_collator)
    ...
    def log_eval_stats(
        ...
        """
        A function that logs all the validation stats.
        ...
        if self.accelerator.is_main_process:
            ...
            logs["env/eval_reward_mean"] = torch.mean(rewards).cpu().numpy().item()
            logs["env/eval_reward_std"] = torch.std(rewards).cpu().numpy().item()
            logs["env/eval_reward_dist"] = rewards.cpu().numpy()
